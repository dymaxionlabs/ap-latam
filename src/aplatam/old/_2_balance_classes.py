...
def cnn_training_dir_structure(train_dir):
    ...
    training_pos_files = set(np.random.choice(pos_files, int(round(len(pos_files) * 0.75)), replace = False))
    training_neg_files = set(np.random.choice(neg_files, int(round(len(pos_files) * aug * 0.75)), replace = False))
    testing_pos_files = set(pos_files) - set(training_pos_files)
    testing_neg_files = set(np.random.choice(list(set(neg_files) - set(training_neg_files)), len(testing_pos_files), replace = False))
    ...
    labels = {'training_pos_files': training_pos_files,
    ...
    for label in list(labels.keys()):
        for fname in labels[label]:
            ...
            if splitted_label[0] == 'training' and splitted_label[1] == 'pos':
                ...
                copyfile(fname, os.path.join('data_keras/train_hires_balanced/vya/{0}'.format(base_fname)))

            if splitted_label[0] == 'training' and splitted_label[1] == 'neg':
                if not os.path.exists('data_keras/train_hires_balanced/no_vya/'):
                    os.makedirs('data_keras/train_hires_balanced/no_vya/')
                copyfile(fname, os.path.join('data_keras/train_hires_balanced/no_vya/{0}'.format(base_fname)))

            if splitted_label[0] == 'testing' and splitted_label[1] == 'pos':
                if not os.path.exists('data_keras/validation_hires_balanced/vya/'):
                    os.makedirs('data_keras/validation_hires_balanced/vya/')
                copyfile(fname, os.path.join('data_keras/validation_hires_balanced/vya/{0}'.format(base_fname)))

            if splitted_label[0] == 'testing' and splitted_label[1] == 'neg':
                if not os.path.exists('data_keras/validation_hires_balanced/no_vya/'):
                    os.makedirs('data_keras/validation_hires_balanced/no_vya/')
                copyfile(fname, os.path.join('data_keras/validation_hires_balanced/no_vya/{0}'.format(base_fname)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('input_dir',
            help='Path where jpg tiles are stored')
