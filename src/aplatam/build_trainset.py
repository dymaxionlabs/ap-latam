...
def build_trainset(rasters, vector, config, temp_dir):
    ...
    for varRaster in rasters:  
        ...
        write_window_tiles(shapes, temp_dir,varRaster, config["size"], config["sep_size"], intensity_percentiles)


def write_window_tiles(shapes, output_dir, tile_fname, size=64, step_size=16, rescale_intensity=True, intensity_percentiles = (2,98)):
