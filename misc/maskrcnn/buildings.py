...
Mask R-CNN
Train on the building outlines segmentation dataset from the
...
Usage: import the module (see Jupyter notebooks for examples), or run from
    ...
    python3 buildings.py train --dataset=/path/to/dataset --weights=imagenet

    # Train a new model starting from specific weights file
    python3 buildings.py train --dataset=/path/to/dataset --weights=/path/to/weights.h5

    # Resume training a model that you had trained earlier
    python3 buildings.py train --dataset=/path/to/dataset --weights=last

    # Generate submission file
    python3 buildings.py detect --dataset=/path/to/dataset --weights=<last or /path/to/weights.h5>

    # Test over an image directory
    python3 buildings.py test --weights=<last or /path/to/weights.h5> images_dir
"""
...
# NOTE: These ids are invalid, they were copied from another MaskRCNN sample
# script...
#
...
class BuildingsConfig(Config):
    """Configuration for training on the buildings segmentation dataset."""
    ...
    # then 0.5 is the minimum anyway as it picks between buildings and BG
...
def train(model, dataset_dir):
    ...
    dataset_train.load_dataset(os.path.join(dataset_dir, "train"))
    dataset_train.prepare()

    # Validation dataset
    dataset_val = BuildingsDataset()
    dataset_val.load_dataset(os.path.join(dataset_dir, "val"))
    dataset_val.prepare()
...
def test(model, dataset_dir):
    ...
    for image_file in image_files:
        ...
        r = model.detect([image], verbose=0)[0]
        # Encode image to RLE. Returns a string of multiple lines
        source_id, _ = os.path.splitext(os.path.basename(image_file))
        rle = mask_to_rle(source_id, r["masks"], r["scores"])
        submission.append(rle)
        # Save image with masks
        visualize.display_instances(
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Mask R-CNN for buildings counting and segmentation')
    parser.add_argument("command",
                        metavar="<command>",
                        choices=['train', 'detect', 'test'])
