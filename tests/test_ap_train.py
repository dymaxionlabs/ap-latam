...
def rasters_with_different_crs(raster):
    return {'a.tif': {'init': 'epsg:4326'},
            'b.tif': {'init': 'epsg:32720'},
            'c.tif': {'init': 'epsg:4326'}}[raster]
...
@patch('aplatam.train.get_raster_crs', return_value=some_crs)
def test_validate_vector_crs_ok(raster_mock, vector_mock, some_rasters, some_vector):
...
def test_validate_vector_crs_fail(raster_mock, vector_mock, some_rasters, some_vector):
    with pytest.raises(RuntimeError):
...
@patch('aplatam.train.get_raster_band_count', side_effect=rasters_with_different_band_count)
